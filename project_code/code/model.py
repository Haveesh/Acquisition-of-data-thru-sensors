...
class User:
   ...
   def setLogfile(self,quizid):
       self.logfile = self.matric +"$"+ quizid +"$"+ datetime.datetime.now().strftime("%H-%M-%S")+".txt"
       return self.logfile
   ...
   def camera_record(self):
        file= self.logfile[:-4]
        subprocess.Popen(['python', 'cam.py', self.pathdir, file])


   def camera_play(self,log):
        print log
        file= log[:-4]
        subprocess.Popen(['python', 'runvideo.py', self.pathdir, file])
