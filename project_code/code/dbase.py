...
class db:
    ...
    def database_insert(self,user,quiznum,file):
        cur = self.con.cursor()
        cur.execute("INSERT INTO subject(name,matric,intw_num,intw_date,logfile,starttime) VALUES(%s,%s,%s,%s,%s,%s);",
                (user.getName(), user.getMatric(), quiznum, datetime.datetime.now().strftime("%Y-%m-%d"), file, str(datetime.datetime.now().strftime("%H:%M:%S:%f"))))
