...
def index():
    if request.method == 'GET':
        ...
        if(session["record"]==True):
            ...
            firstime = db_user.getQuesStartTime(session['user_matric'], session['quiz_id'], session['quesid'][0])
            start = db_user.getQuesStartTime(session['user_matric'], session['quiz_id'], session['quesid'] [int(session['startq'])-1])
            end = db_user.getQuesEndTime(session['user_matric'], session['quiz_id'], session['quesid'][int(session['endq'])-1])
            starttime = str(int(start) - int(firstime))
            endtime = str(int(end) - int(firstime))

            user.playback(log,starttime,endtime)
            user.camera_play(log)
